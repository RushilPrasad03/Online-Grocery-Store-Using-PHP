...
// get search results
if($_SESSION['filter']!=''){
    $searchDisplay = $_SESSION['filter']." AND `product` LIKE '%$search%'";
    $_SESSION['filter']='';
}
else{
    $searchDisplay = "SELECT * FROM `product` WHERE `product` LIKE '%$search%'";
}
$searchDisplayResult = $mysqli->query($searchDisplay);
...
<body>
    ...
    <div class="heading">
        ...
        <p class="heading-url"><a href="index.php">home </a>>> <a href="shop.php">shop </a>>> search </p>
    </div>
    <section class="products-all">
        <h1 class="title"> search <span>results</span> <a><?php echo mysqli_num_rows($searchDisplayResult)?> results</a> </h1>
        <div class="products-body">
            ...
            <div class="box-container">
                <!-- DB DISPLAY !!!! -->
                <?php
                    while($rows=$searchDisplayResult->fetch_assoc()){
                ?>
                ...
                <form action="function/cart-change.php" method="POST" class="box">
                    ...
                    <div class="content">
                        <h3><?php echo $rows['product']; ?></h3>
                        <div class="price">₹<?php echo $rows['discount']; ?><span class="discount">₹<?php echo $rows['price']; ?></span></div>
                        <div class="cart-add">
                            <input type="hidden" name="product" value="<?php echo $rows['product_id']; ?>">
                    <?php
                        $product = $rows['product_id'];
                        $user = $_SESSION['user_id'];
                        $check = "SELECT * FROM `cart` WHERE `user_id`='$user' AND `product_id`='$product'";
